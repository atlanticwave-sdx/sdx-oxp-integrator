...
class Port(Model):
    ...
    def port_id(self, port_id):
        ...
        if port_id is None:
            raise ValueError(
                "Invalid value for `port_id`, must not be `None`"
            )  # noqa: E501
        if port_id is not None and not re.search(
            r"^((urn:sdx:port:)[A-Za-z_.-]*$)", port_id
        ):  # noqa: E501
            raise ValueError(
                "Invalid value for `port_id`, must be a follow pattern or equal to `/^((urn:sdx:port:)[A-Za-z_.-]*$)/`"
            )  # noqa: E501
